...
export default function ChatbotClient({ initialConfig, subdomain }) {
  ...
  return (
    <div className="flex h-screen bg-gray-50 relative">
      {/* Slim Left Navbar - Hidden on Mobile */}
      <nav className="hidden md:flex md:w-16 bg-white border-r border-gray-200 flex-col items-center py-4 space-y-8">
      ...
      <div className="flex-1 flex flex-col" style={{ backgroundColor: customization.backgroundColor }}>
        ...
        {/* Hero Section */}
        <div className="px-4 md:px-8 py-6 md:py-16 text-center">
          <h1 
        ...
        {/* Chat Interface */}
        <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full px-2 md:px-4 pb-4 md:pb-8">
          <div className="flex-1 bg-white rounded-t-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="h-full p-3 md:p-6 space-y-4 overflow-y-auto min-h-[300px] md:min-h-[400px] max-h-[500px] md:max-h-[600px]">
              {messages.length === 0 ? (
                ...
                messages.map((message, index) => (
                  ...
                  >
                    <div
                      className={`max-w-[85%] md:max-w-[80%] rounded-2xl px-4 md:px-6 py-2 md:py-3 ${
                        message.role === "user"
                          ? "text-white"
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-gray-100 rounded-2xl px-4 md:px-6 py-2 md:py-3 text-gray-500">
                    Thinking...
                  </div>
                </div>
              )}
          ...
          {/* Input Form */}
          <form onSubmit={sendMessage} className="bg-white border border-t-0 border-gray-200 rounded-b-xl p-2 md:p-4">
            <div className="flex gap-2 md:gap-3">
              <input
                ...
                className="flex-1 px-4 py-2 md:py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                disabled={isLoading || tokenUsage?.remaining <= 0}
              />
              <button
                type="submit"
                disabled={isLoading || tokenUsage?.remaining <= 0}
                className="px-4 md:px-6 py-2 md:py-3 text-white rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {/* Error Message */}
        {error && (
          <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 w-[90%] md:w-auto">
